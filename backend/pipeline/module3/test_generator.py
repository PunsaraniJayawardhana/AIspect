...
def _build_template_defect_script(discrepancy, app_url, source_hints=None, nav_path=""):
    ...
    # Missing/wrong-label discrepancies should fail when required text is absent in the live UI.
    if "missing" in dtype or "label" in dtype:
        if use_phrase_match:
            assertion = (
                f"    const expectedPhrase = {selected_candidate_literal};\n"
                "    cy.get('body', { timeout: 10000 }).invoke('text').then((bodyText) => {\n"
                "      const haystack = (bodyText || '').toLowerCase();\n"
                "      const found = haystack.includes((expectedPhrase || '').toLowerCase());\n"
                "      expect(found, `Expected phrase: ${expectedPhrase}`).to.eq(true);\n"
                "    });\n"
            )
        else:
            assertion = (
                f"    const expectedKeyword = {selected_candidate_literal};\n"
                "    cy.get('body', { timeout: 10000 }).invoke('text').then((bodyText) => {\n"
                "      const haystack = (bodyText || '').toLowerCase();\n"
                "      const found = haystack.includes((expectedKeyword || '').toLowerCase());\n"
                "      expect(found, `Expected keyword: ${expectedKeyword}`).to.eq(true);\n"
                "    });\n"
            )
    else:
        # Generic fallback still checks that discrepancy-related content is discoverable.
        if use_phrase_match:
            assertion = (
                f"    const expectedPhrase = {selected_candidate_literal};\n"
                "    cy.get('body', { timeout: 10000 }).invoke('text').then((bodyText) => {\n"
